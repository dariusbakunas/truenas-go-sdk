...
```golang
ctx := context.WithValue(context.Background(), angelfish.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), angelfish.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), angelfish.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), angelfish.ContextOperationServerVariables, map[string]map[string]string{
